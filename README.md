...
| `brief` + `brief_<part>` | The [per-wake brief](#run-under-a-router-wake-mode), broken down by the part that produced it: `now`, `brain`, `budget`, `initialize`, `manifest`, `defects`, `safety`, `mcp`, `dashboard`, `memory`, `system_prompt`. A part your config doesn't compose is simply absent, never a zero |
