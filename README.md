...
A wake reconciles **every** kind of unseen item on the timeline, not just messages. The case the timeline scan would otherwise miss is an **inbound webhook delivery**: a received `webhook_event` is not a timeline item the way a message or an activated task is, so a woken agent never sees one unless it goes looking. The wake does that lookup — surfacing unseen webhook events under their own high-water mark, with the same idempotency the message path has — so a peer woken on `webhook_event.received` **perceives and can act on the delivery**. The router passes `--event <uuid>` (the delivery that woke it) so the first wake acts on exactly that event rather than baselining it; without a trigger, a first wake only baselines, so a fresh agent never replays a backlog of historical deliveries. (Managing the endpoints and reading event details is the [webhook tools](#receive-inbound-activity--the-webhook-tools); this is the *perceiving it on wake* half.)
