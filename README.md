...
Compacting at **half** the ceiling is only safe because **no single turn can leap the gap**: every step of a turn may run a tool, and each call persists at most 4 KB of result plus 2 KB of arguments, so one turn adds at most `6144 × HARNESS_MAX_STEPS` characters — about **49,152 tokens** at the shipped 24-step budget. That has to fit in the headroom *above* the threshold, which makes the guarantee an inequality with **two knobs that break it from opposite sides**:
...
threshold, below the 49152 a single tool-heavy turn can add (6144 chars of result + arguments x 24
steps at 3.0 chars/token): a turn may overshoot the ceiling before compaction, which runs only
