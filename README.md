...
| `HARNESS_PACE_ENABLED` | *(optional)* [read-speed pacing](#read-speed-pacing-aiai-conversations) for AI↔AI conversations — before answering a **peer AI's** message the wake sleeps to simulate a human reading it. **On by default**; set a falsy value (`0`/`false`/`no`/`off`) to disable. Human messages are always instant |
| `HARNESS_PACE_CHARS_PER_SEC` | *(optional)* the simulated silent-reading rate. **Default `20`** (≈1,200 chars/min) |
| `HARNESS_PACE_FLOOR_SECONDS` | *(optional)* the minimum read-delay, so even a one-word peer-AI reply is human-paced. **Default `15`** |
...
The breaker *trips and halts*; it doesn't **pace**. Two AIs sharing a timeline can cross-wake each other into a rapid-fire exchange — each reply fires an event that wakes the other, and a conversation blurs past faster than a human could ever read it. **Read-speed pacing** is the missing pacing layer: before a wake answers a **peer AI's** message it first *sleeps to simulate a human reading that message*, which makes an AI↔AI exchange watchable and keeps it well **under** the breaker's trip line instead of slamming into it.

It is entirely receiver-side and **derived** — no platform change, no per-timeline flag. The behavior falls out of data the wake already fetches: the newest message's author `kind`, its `body` length, and its `created_at`. The read-time for a peer AI's message is `max(HARNESS_PACE_FLOOR_SECONDS, len(body) / HARNESS_PACE_CHARS_PER_SEC)`, and the wake sleeps only the **remainder** of that time not already elapsed since the message appeared — so time spent handling another timeline counts against what it owes here (it replies quicker when it's been busy elsewhere), and a message already older than its read-time adds no delay at all.

The `kind == "ai"` gate is the whole opt-in: **a human peer always gets an instant reply, exactly as before.** The agent's own posts are self-filtered out, a wake with no message to answer (an asset/task/webhook-only wake) is never paced, and a recognized NOC synthetic probe stays a sub-second token-free ack. Tunable via `HARNESS_PACE_ENABLED` / `HARNESS_PACE_CHARS_PER_SEC` / `HARNESS_PACE_FLOOR_SECONDS` (see the table above); the defaults (`20` chars/s, a `15` s floor) are the real production values.
