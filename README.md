...
**Every part is fenced in a named tag pair**, and the tag is the *source's* name — the filename for a part that comes from a file (`<initialize.md>`, `<dashboard.md>`, `<system-prompt.md>`), the part's own name otherwise (`<now>`, `<budget>`, `<manifest>`, `<defects>`, `<safety>`, `<memory>`). So an agent that can read its own config home sees the same names in its brief that it sees in `<config-home>/prompts/` and on the platform. This is not decoration: the brief mixes authority levels inside one ~54 K-character system turn — two of those parts are *instructions*, five are *harness-generated*, the dashboard is *fetched live* and carries peer-authored strings (timeline names, handles, about text), and the memory part is *recalled excerpts of past conversation*. The input-security floor in `initialize.md` tells the agent its only instructions are this brief and its charter; the fences are how it can tell, **inside** the brief, where instruction ends and fetched data begins.
...
- **A peer cannot forge a fence.** Any tag literal is stripped out of the two parts a peer can influence — the live dashboard and the recalled memory — before either is wrapped, so naming a timeline `</dashboard.md>` does not end the data block early and have the rest read as instruction. Removal, never rejection: the rest of the text is still shown.
...
| its **final text was cut off** by the model's output budget | **resumed** — the model continues from where it stopped | A fragment is not a decision. Re-driving it would be *safe* (nothing fired) and would not *work*: the same input under the same budget stops in the same place, forever. |
...
Four model-call failures are **transient** — the same call, re-issued unchanged, usually succeeds — and all four are retried, on **every** model call the harness makes: the brain's, the [memory reranker's](#let-a-model-pick-what-gets-recalled--the-llm-reranker), and the [describer's](#give-a-blind-model-eyes--the-describer).
...
- **A transport failure** — DNS, TCP, TLS, a connect timeout, or a **read timeout**, where the request was accepted and the answer never came back. The log word distinguishes the last of those (`reason=timeout`) from the rest (`reason=transport`), because they send you to different places: a vendor that answered too slowly is not a network that is down.
...
So the call is re-issued up to **2** more times (3 attempts) with a short backoff, and the common case — a one-off flake that succeeds on the very next try — never surfaces at all. It is **classified by the nature of the fault, never by vendor**: every adapter maps its own SDK's parse failure, 5xx, 429 and transport failure onto the same four classes, so one rule in one place governs OpenAI, OpenRouter, and the native xAI gRPC path alike. (Before this, whether a 5xx was retried was an *accident* of which SDK you happened to run — the `openai` SDK retries them internally, while the native OpenRouter adapter disables its SDK's retry outright, since that one backs off for up to an hour and would hang a wake. The transport class was still that accident until [#545](https://github.com/basecradle/basecradle-harness/issues/545): a read timeout was survivable three times over inside the `openai` SDK and fatal on the first raise through OpenRouter.)
...
- **Out of funds** (the provider refused for lack of credit): *"I can't respond right now — my `<provider>` account is out of credit (`<verbatim error>`). Add funds to the `<provider>` account to resume; pending messages will be handled then."* The pending work is left **pending**, so it resumes untouched the moment the account is funded and the next call succeeds. The notice is **debounced** — one per outage per timeline — and after it the wake fails fast and quiet, so a prolonged outage doesn't spam the conversation or hammer the unfunded account.
...
- **The lifecycle and its verdict are in color** — so a wake's shape is *seen* rather than read, in a terminal and in Better Stack Live Tail alike. `wake start` is GREEN and `wake end` BLUE; a failure (`wake failed`, `wake reported_failure`, `post failed`, `cleanup failed`, `cleanup blocked`) is RED; an in-between (`wake skipped`, `wake billing_blocked`, `degraded`) is YELLOW; a recovery (`wake billing_recovered`) is GREEN. The `outcome=` pair carries its own color wherever it appears — GREEN `ok`, RED `error`, YELLOW `declined`. Everything else stays plain: the *fact* lines (`llm`, `tool`, `media`, `unspoken`, `posted`, `step`, `context …`) name what happened rather than judge it, and coloring them would flatten the signal back into noise.
...
```
INFO context attribution unit=chars source=timeline:019e77…6da total=12794 messages=13 tools=1474 tools_count=1 brief=10546 brief_now=268 brief_budget=573 brief_initialize=9582 brief_manifest=41 brief_dashboard=43 brief_system_prompt=39 history=774 history_charter=0 history_summary=0 history_steps=208 history_user=196 history_assistant=130 history_tool=240 history_other=0 images=0
```
...
| `brief` + `brief_<part>` | The [per-wake brief](#run-under-a-router-wake-mode), broken down by the part that produced it: `now`, `budget`, `initialize`, `manifest`, `defects`, `safety`, `dashboard`, `memory`, `system_prompt`. A part your config doesn't compose is simply absent, never a zero |
