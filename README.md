...
| `HARNESS_RESPONSE_RETRIES` | *(optional)* how many **extra** times the engine re-requests a provider call whose [response came back truncated / unparseable](#retrying-a-truncated-provider-response) (the "EOF while parsing a value" class) before the wake gives up. **Default `2`** (up to 3 total attempts); `0` disables the retry. Only that transient class is retried — a connection, auth, rate-limit, or config error is never re-tried |
...
The whole persisted transcript is replayed to the model on **every** wake, so anything written into it is paid for again on every future wake, forever. Two rules keep that bill honest — and they are the difference between a transcript that grows with the *conversation* and one that grows with the *mechanism*. (An agent running without them reached **754,201 input tokens per model call** in three days of ordinary activity; 47% of that context was stale copies of its own brief and 39% was raw tool output, against 1.6% actual dialogue.)
...
- **Tool results are read in full, kept capped.** The model sees a tool's complete output on the turn it ran. What *persists* is the first 2 KB and last 0.5 KB around an elision marker naming the original size (`[... 137,412 chars elided of 145,984 ...]`), for any result over 4 KB. Otherwise a single mailbox listing or wide file read is a permanent tax on the life of the timeline. The result message is *edited*, never dropped, so its `tool_call_id` pairing stays intact. This is the same cost discipline the engine already applies to a [viewed image](#see-hear-and-make-media--the-media-tools) — seen once, never re-billed — extended to text. A transcript written before the cap existed heals the first time it is loaded.
...
### Retrying a truncated provider response

A model call can come back **truncated or unparseable** — the body arrived but the SDK can't turn it into a turn (the "EOF while parsing a value" class, seen intermittently on long responses). Left unhandled this is worse than it sounds: a wake marks each item *seen* **before** it calls the model, so a wake that aborts on the bad response **silently drops the peer's message** — no reply, and no later wake to retry it.

So the engine treats that one failure class as **transient and retryable**: it re-requests the completion up to `HARNESS_RESPONSE_RETRIES` times (**default 2**, i.e. up to 3 attempts) with a short backoff before giving up, and the common case — a one-off flake that succeeds on the very next try — never surfaces at all. It is **capability-based, not provider-specific**: every adapter maps its own SDK's parse/validation failure to the same class, so the retry fires identically whether the truncated body came from OpenAI, an OpenRouter model, or the native xAI gRPC path — never a one-provider special case. Only that class retries; a connection drop, an auth or rate-limit error, or a permanent config error (a bad `model_params.json` key) is **never** re-tried, because re-issuing it would only repeat it. When the retries *are* exhausted the wake still aborts — but only after logging a `WARNING` per attempt and a final `ERROR` naming the failure class and the attempt count, so a genuinely-wedged provider leaves a diagnosable trail instead of a silent drop. Set `HARNESS_RESPONSE_RETRIES=0` to disable the retry (a single attempt).
