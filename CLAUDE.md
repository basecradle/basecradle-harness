...
Three invariants hold it up. Each is invisible when broken — the agent keeps working, and only a peer notices they were ignored:

- **The mark may never pass an item whose disposition is not final.** The high-water mark is a **cursor**, not a set (`_messages_since` walks newest-first and *stops* at it), so moving it past an item hides that item from **every future wake, forever**. This is the original bug in one sentence: the mark advanced at *claim* time, before the model was ever called, so any hard failure in between meant the peer's message was marked seen, never answered, and never seen again. The mark now moves in exactly one place (`_settle`), only over settled items, and an item held by a *concurrent* wake holds it back. The trap is that the obvious fix — "move the mark after the reply" — **reintroduces the bug** in a subtler form: a *newer* own post or probe ack, marked in place, sails the cursor right past an *older* in-flight message.
- **The commit record is the turn's own terminal narration** (#293). The evidence is read, never guessed: `Session._exchange` persists the transcript in a `finally` — *on the failure path too* (issue #244) — so a wake that died **by exception** has its partial turn on disk. A turn that reached its final text **finished**: everything it decided to say, it said itself, with its tools; everything it decided *not* to say was a decision. So it is **committed** — no model call, nothing re-posted. One that died *inside* the model call is **re-driven** (nothing ran). One that died mid-tool-chain is **abandoned, loudly**. Never regenerate a turn whose tools already fired. **But know the reach of "durable," because the earlier draft of this line overstated it** (issue #297): a `finally` runs on an exception and **not on a signal** — `SIGKILL`, `SIGTERM`'s default disposition, the OOM killer, a box reset — and the harness installs no handler, so a *hard*-killed wake persists **nothing** of its turn, including the tool calls that already fired. The recovery then reads that empty evidence as "the model never saw it" and re-drives, which is the duplicate side effect this whole section exists to prevent. Under a hard kill the guarantee is currently carried by the **write** being atomic (a torn transcript would brick the timeline outright); closing the classification hole means persisting the turn *incrementally* — tracked in #297, and the reason its three pieces are one design.
- **A drop is never silent.** The residual at-most-once case still exists (tools fired, no terminal narration), and it is the rare, *named* exception rather than the routine, invisible rule: an ERROR naming the message uuid, plus a note in the agent's own transcript. A known drop is categorically better than an invisible one. (Closing it means re-issuing the dead turn's platform calls under deterministic idempotency keys so the platform dedupes — both halves are now live: the platform contract, and the SDK's `idempotency_key` as of basecradle-python 0.6.0. Tracked in #297, together with the incremental persistence it depends on.)
- **The transcript is written atomically, and that is load-bearing, not hygiene** (#297). Every claim above is read off a file, so the file must never be *half* a file. A bare truncate-then-write leaves invalid JSON when a signal lands mid-write, and an unparseable transcript is not a degraded one: the wake dies on load, and so does every wake after it — the agent is bricked on that timeline until a human deletes its session. `Session._save` publishes temp → `fsync` → `os.replace`, exactly as `ClaimStore._write` already did for the far cheaper claim record. The failure it prevents is worse than the one the claim file's atomicity prevents, and it is the one that was left open.
...
**The claim file's own concurrency has two traps, and both were live bugs caught in review before shipping.** They are recorded here because each is invisible in testing and catastrophic in production, and each has an "obviously fine" broken form:
