...
Two things follow from the compaction half that are easy to break by accident, so they are stated here rather than left in the code: **the tool-result cap is a load-bearing prerequisite of the compaction threshold, not a neighbor of it** — compaction fires *between* turns, so it is only safe to sit at 50% of the ceiling because one turn's persisted growth is bounded (4 KB × 24 steps ≈ 30 K tokens) and therefore cannot leap the gap; raise the cap or the step budget far enough and the threshold stops being a safe distance. And **a compaction cut may land only immediately before a *real* `user` turn** — a cut mid-tool-chain strands a `tool` result whose assistant call was dropped, and a dangling `tool_call_id` is malformed *permanently*, breaking every later wake. When no safe cut exists the compactor declines and says so; that is the correct failure, and a "fix" that makes it cut anyway is the worst bug in this file. *"Real"* is doing work in that sentence (#297): the engine and the code bridge both inject `user`-role turns that are a turn's own **work**, not a new turn of the conversation, and an injected turn treated as a boundary can become the "newest user turn always survives" **floor** — at which point compaction keeps the image caption and summarizes the peer's actual message away. Valid transcript, broken agent: the delivery guarantee's recovery can no longer find the turn that carried a message. `Message.injected` marks them; `_cut_index` skips them.

**One known, stated gap — a tool call's `arguments` persist whole, forever.** `_payload` caps a `tool` result's *content*; it does not touch the `tool_calls` on the assistant turn that asked for it. So an `assets create` carrying a 200 KB document re-sends that document to the model on every future turn for the life of the timeline. It is pre-existing, it is a real violation of the first invariant above, and it is **not** fixed here on purpose: the resume *replays* an interrupted platform create from exactly those arguments, so capping them naively would break the recovery. Closing it means capping the arguments of calls that are **not** replayable creates, or storing the payload out of band. Tracked; named here so the next reader does not think nobody looked.
