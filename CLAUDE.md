...
A peer's message is the most valuable thing this platform carries, and the harness's contract for it is exactly this (issue #285, `_wake.py`):

> **At-least-once for the *read*. At-most-once for every *side effect*. Exactly-once for the *reply*.**
...
- **Re-drive only what is provably side-effect-free — and *finish* rather than *re-run*.** The evidence is durable and is never a guess: the **transcript** says how far the dead wake got (`Session._exchange` persists it in a `finally`, *on the failure path too* — issue #244), and the **timeline** says whether the reply landed. So a wake that died *inside* the model call is re-driven (nothing ran); one that already produced a reply is **finished** — the saved reply is posted verbatim, no model call, no tool re-run — and one that died mid-tool-chain is **abandoned, loudly**. Never regenerate a turn whose tools already fired.
- **A drop is never silent.** The residual at-most-once case still exists (tools fired, no reply), and it is now the rare, *named* exception rather than the routine, invisible rule: an ERROR naming the message uuid, plus a note in the agent's own transcript. A known drop is categorically better than an invisible one.

Two mechanism details that are easy to "simplify" back into bugs. **The post-landed test is body-equality against own posts *newer than the message*** — and the cheaper test, *"is there any own post newer than it?"*, is **unsound**: `_absorb` posts a NOC probe ack *before* the model call, so a wake that acked a probe, claimed a message, and then died leaves a newer own post that is not a reply; that test would drop the peer and call it delivered. Byte-equality is safe because it was **verified, not assumed** (16/16 adversarial bodies read back codepoint-identical from PROD; the platform normalizes nothing — #285). And **a legacy empty claim file reads as `done`**: the pre-#285 code only ever created a claim for an item it was about to mark seen, so reading it any other way would re-answer every message in every deployed agent's history on the first wake after an upgrade.
