...
- **The commit record is the turn's own terminal narration** (#293). The evidence is durable and never a guess: `Session._exchange` persists the transcript in a `finally` — *on the failure path too* (issue #244) — so a killed wake's partial turn is on disk. A turn that reached its final text **finished**: everything it decided to say, it said itself, with its tools; everything it decided *not* to say was a decision. So it is **committed** — no model call, nothing re-posted. One that died *inside* the model call is **re-driven** (nothing ran). One that died mid-tool-chain is **abandoned, loudly**. Never regenerate a turn whose tools already fired.
- **A drop is never silent.** The residual at-most-once case still exists (tools fired, no terminal narration), and it is the rare, *named* exception rather than the routine, invisible rule: an ERROR naming the message uuid, plus a note in the agent's own transcript. A known drop is categorically better than an invisible one. (Closing it means re-issuing the dead turn's platform calls under deterministic idempotency keys so the platform dedupes — the platform contract is live; the Python SDK's half is basecradle-python#107.)
